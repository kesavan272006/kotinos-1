...
type FlowerMenuProps = {
  menuItems?: MenuItem[]; // optional to allow default
  iconColor?: string;
  ...
  return (
    <>
      ...
      <label
        ...
        className="absolute inset-0 z-20 m-auto flex cursor-pointer items-center justify-center rounded-full transition-all hover:scale-110"
        style={{
          backgroundColor,
          color: iconColor,
...
const MenuItemComponent = ({
  ...
  return (
    ...
    >
      <Link
        to={item.href}
        className={`flex h-full w-full items-center justify-center rounded-full opacity-90 transition-all duration-100 ${
      ...
      >
        <Icon
          className="transition-transform duration-200 scale-110 group-hover:scale-[1.3]"
          style={{ width: iconSize, height: iconSize }}
        />
      </Link>
    </li>
...
const FlowerMenu: React.FC<FlowerMenuProps> = ({
  menuItems = [], // Default fallback
  iconColor = "white",
  ...
  // Optional: early return if no menu items
